...
fn bench_load(label: &str, filename: &str, cols: Option<&[String]>, runs: usize) {
    ...
    let min_ms = times.iter().map(|d| d.as_secs_f64() * 1000.0).fold(f64::MAX, f64::min);
    println!("{:30} avg={:.1}ms  min={:.1}ms  ({}x)", label, avg_ms, min_ms, runs);
}
...
fn bench_projection_at_csv() {
    ...
    let headers: Vec<String> = {
        ...
        first_line.split(';').skip(1).map(|s| s.trim().to_string()).collect()
    };
