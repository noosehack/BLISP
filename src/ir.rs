...
pub enum NumericFunc {
    ...
    /// Canonical: SHF_REC_NLN_LOCF (Shift-equivariant, Recursive, Nonlinear, LOCF)
    ///
    /// Contract:
    ...
    /// Canonical: MSK_WKE (Mask, Weekend)
    ///
    /// Contract:
    ...
    /// Canonical: SHF_PFX_LIN_SUM (Shift-equivariant, Prefix, Linear, Sum)
    ///
    ...
    /// Canonical: SHF_WIN_LIN_AVG (Shift-equivariant, Window, Linear, Average)
    ///
    ...
    /// - Shape preserved (I1-I3)
    /// - NA mask monotone
    SHF_WIN_LIN_AVG { w: usize },
    /// Rolling standard deviation: trailing window std (population, ddof=0)
    /// Canonical: SHF_WIN_NLN_SDV (Shift-equivariant, Window, Nonlinear, StdDev)
    ///
    /// Contract (see contracts.md §5):
    /// - Trailing window: [i-w+1 .. i] inclusive
    /// - Skip NA in window, require w valid values (strict min_periods)
    ...
    /// - Shape preserved (I1-I3)
    /// - NA mask monotone
    SHF_WIN_NLN_SDV { w: usize },
    /// Rolling mean (min 2 observations): relaxed min_periods for masked calendars
    /// Canonical: SHF_WIN_MIN2_LIN_AVG (Shift-equivariant, Window, Min2, Linear, Average)
    ///
    /// Contract:
    /// - Trailing window: [i-w+1 .. i] inclusive
    ...
    /// - Prefix i < w-1 always NA
    /// - Shape preserved (I1-I3)
    SHF_WIN_MIN2_LIN_AVG { w: usize },
    /// Rolling std (min 2 observations): relaxed min_periods for masked calendars
    /// Canonical: SHF_WIN_MIN2_NLN_SDV (Shift-equivariant, Window, Min2, Nonlinear, StdDev)
    ///
    /// Contract:
    /// - Trailing window: [i-w+1 .. i] inclusive
    /// - Skip NA in window, require ≥2 valid values (relaxed, not strict w)
    ...
    /// - Prefix i < w-1 always NA
    /// - Shape preserved (I1-I3)
    SHF_WIN_MIN2_NLN_SDV { w: usize },
    /// Rolling mean (min 2 observations) excluding current observation - for ft-zscore
    /// Canonical: SHF_WIN_MIN2_LIN_AVG_EXCL (Shift, Window, Min2, Linear, Avg, ExclCurrent)
    ///
    /// Contract:
    ...
    SHF_WIN_MIN2_LIN_AVG_EXCL { w: usize },
    /// Rolling std (min 2 observations) excluding current observation - for ft-zscore
    /// Canonical: SHF_WIN_MIN2_NLN_SDV_EXCL (Shift, Window, Min2, Nonlinear, StdDev, ExclCurrent)
    ///
...
pub enum SchemaOp {
    ...
    /// Canonical: SHF_PTW_LIN_SPR (Shift-equivariant, Pointwise, Linear, Spread)
    ///
    ...
    /// Canonical: MSK_WKE_DEF (Mask, Weekend, Define)
    ///
    /// Contract:
    /// - I1 preserved: index Arc ptr_eq
...
mod tests {
    ...
    fn test_join_op() {
        ...
        let join = plan.add_node(Node {
            id: NodeId(2),
            op: Operation::Join(JoinOp::ALIGN { x, y }),
            schema: SchemaInfo {
                index_type: Some(IndexType::Date),
                colnames: None,
